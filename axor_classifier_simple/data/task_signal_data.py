...
_PREFIXES_RU = [
    "", "", "", "",                              # most have no prefix
    "пожалуйста, ", "срочно: ", "слушай, ", "так, ",
    "привет! ", "коллеги, ", "быстрый вопрос: ", "кстати, ",
]


def _augment_ru(text: str, rng: random.Random) -> str:
    """Russian counterpart of _augment: prefixes, case, punctuation.
    No typo injection — the English typo table does not transfer."""
    if rng.random() < 0.12:
        text = rng.choice(_PREFIXES_RU) + text
    r = rng.random()
    if r < 0.06:
        text = text.upper()
    elif r < 0.12:
...
# ── Russian template corpus ────────────────────────────────────────────────────
#
# Same design principles as _TEMPLATES, in Russian. Placeholders reuse the
# shared substitution vocabulary (file/function/service names stay Latin —
# that is how real code-mixed Russian requests look).
#
# Scope markers:
#   focused:   "просто", "быстро", "один", "эту", "небольшой"
#   moderate:  "модуль {module}", "все ... в {module}", "несколько"
#   expansive: "весь", "целиком", "все", "каждый", "по всему проекту", "с нуля"
#
# Verb sets per nature:
#   generative: напиши, создай, добавь, реализуй, сделай, сгенерируй, набросай
#   mutative:   исправь, почини, поправь, обнови, измени, перепиши, замени,
#               переименуй, убери, отрефактори
#   readonly:   объясни, покажи, опиши, проанализируй, найди, проверь,
#               посмотри, разбери, что делает, как работает, почему

_TEMPLATES_RU: dict[tuple[str, str, str], list[str]] = {

    # ── FOCUSED + MUTATIVE ──────────────────────────────────────────────────────

    ("focused", "mutative", "coding"): [
        "исправь баг в {func}",
        "исправь ошибку в {file}",
        "почини {func}",
        "почини сломанный тест",
        "поправь опечатку в {file}",
        "обнови {func}, чтобы обрабатывался null",
        "убери мёртвый код из {file}",
        "переименуй {func} в {func2}",
        "измени возвращаемый тип {func}",
        "исправь падающий тест",
        "поправь импорт в {file}",
        "просто исправь опечатку",
        "быстро почини баг в {func}",
        "убери лишний print из {func}",
        "исправь race condition в {func}",
        "замени захардкоженное значение в {func}",
        "сделай {func} асинхронной",
        "тест падает, почини его",
        "поправь регулярку в {func}",
        "добавь недостающую проверку на null в {func}",
    ],

    ("focused", "mutative", "support"): [
        "почини мой конфиг {service}",
        "исправь строку подключения",
        "поправь порт в конфиге",
        "обнови пароль в конфиге",
        "исправь неправильный hostname в {config_file}",
        "поменяй таймаут для {service}",
        "почини сломанную переменную окружения",
        "исправь синтаксическую ошибку в {config_file}",
        "обнови версию {service} в зависимостях",
        "поправь уровень логирования",
        "перезапусти упавший {k8s}",
        "исправь лимиты ресурсов в {k8s}",
        "поправь тег образа в {k8s}",
        "измени число реплик в {k8s}",
        "почини {infra} скрипт деплоя",
    ],

    ("focused", "mutative", "general"): [
        "исправь опечатку",
        "поправь это предложение",
        "измени это слово",
        "исправь грамматику в этом предложении",
        "обнови дату в этом документе",
        "поправь заголовок",
        "перефразируй это предложение",
        "сократи этот абзац",
        "исправь битую ссылку",
        "обнови номер версии",
        "поправь имя в этом документе",
        "исправь орфографическую ошибку",
    ],

    # ── FOCUSED + GENERATIVE ────────────────────────────────────────────────────

    ("focused", "generative", "coding"): [
        "напиши тест для {func}",
        "добавь юнит-тест для {func}",
        "напиши докстринг для {func}",
        "создай мок для {class_name}",
        "реализуй {func}",
        "добавь {func} в {class_name}",
        "напиши небольшой хелпер, который {action}",
        "добавь лог в {func}",
        "сгенерируй заглушку для {func}",
        "создай фикстуру для {class_name}",
        "добавь аннотацию типов в {func}",
        "напиши параметризованный тест для {func}",
        "набросай скелет {func}",
        "создай датакласс для этой структуры",
        "добавь декоратор для {func}",
    ],

    ("focused", "generative", "support"): [
        "покажи, как настроить {service}",
        "дай пример конфига для {service}",
        "напиши минимальный конфиг для {service}",
        "дай команду для перезапуска {service}",
        "покажи, как подключиться к {service}",
        "напиши однострочник для запуска {service}",
        "дай пример {config_file}",
        "покажи, как установить {service}",
        "напиши простой health check для {service}",
        "дай команду kubectl, чтобы посмотреть {log}",
        "покажи манифест {k8s} с одной репликой",
        "дай команду {infra} для отката",
    ],

    ("focused", "generative", "general"): [
        "напиши короткое описание {topic}",
        "набросай краткую заметку о {topic}",
        "создай определение {term} в одну строку",
        "напиши быстрое письмо про {topic}",
        "сделай короткий список плюсов и минусов {topic}",
        "напиши краткое объяснение {term}",
        "набросай короткий статус по {topic}",
        "напиши запись в changelog для этого изменения",
        "создай короткое описание для README",
        "напиши краткий анонс про {topic}",
        "набросай пункт повестки про {topic}",
        "напиши короткую биографию для конференции",
    ],

    # ── FOCUSED + READONLY ──────────────────────────────────────────────────────

    ("focused", "readonly", "coding"): [
        "объясни, что делает {func}",
        "что делает {func}",
        "что делает {file}",
        "покажи, как работает {func}",
        "опиши интерфейс {class_name}",
        "какой возвращаемый тип у {func}",
        "найди, где вызывается {func}",
        "какие аргументы принимает {func}",
        "посмотри на {func} и скажи, что она делает",
        "проверь, обрабатывает ли {func} null",
        "почему {func} возвращает None",
        "зачем нужен {file}",
        "где определён {class_name}",
        "разбери, как устроен {func}",
        "покажи сигнатуру {func}",
    ],

    ("focused", "readonly", "support"): [
        "почему {service} возвращает {error}",
        "что значит {error}",
        "объясни эту ошибку: {error}",
        "почему у меня {error}",
        "что вызывает {error}",
        "почему не работает {func}",
        "почему мой код сломан",
        "что здесь не так",
        "почему {service} не подключается",
        "как прочитать этот стектрейс",
        "почему {k8s} в CrashLoopBackOff",
        "что говорят {log}",
        "почему упал деплой в {env}",
        "почему {k8s} завис в pending",
        "что стало причиной {incident}",
    ],
    ...
    ("focused", "readonly", "general"): [
        ...
        "что значит {term}",
        "как работает {topic}",
        "дай быстрое объяснение {term}",
        "в чём разница между {term} и {topic}",
        "объясни мне {term}",
        "для чего используется {topic}",
        "когда стоит использовать {term}",
        "какие компромиссы у {term}",
        "что такое {topic} простыми словами",
        "зачем нужен {doc_type}",
    ],

    # ── MODERATE + MUTATIVE ─────────────────────────────────────────────────────

    ("moderate", "mutative", "coding"): [
        "отрефактори модуль {module}",
        "перепиши парсер {module}, чтобы обрабатывались краевые случаи",
        "обнови все вызовы устаревшего API {func}",
        "разбей {file} на модули поменьше",
        "исправь все ошибки типов в {module}",
        "обнови все тесты в {module}",
        "замени все использования {func} на {func2}",
        "почини сломанные тесты в {module}",
        "переведи {module} с колбэков на async",
        "наведи порядок в модуле {module}",
        "приведи логирование в {module} к единому формату",
        "обнови схему базы и прогони миграции",
        "убери все устаревшие методы из {class_name}",
        "исправь все ошибки линтера в {module}",
        "переименуй все вхождения {func} в {func2}",
    ],

    ("moderate", "mutative", "support"): [
        "разреши все конфликты зависимостей в проекте",
        "обнови все настройки {service} под новую версию",
        "мигрируй мой конфиг с v1 на v2",
        "обнови все переменные окружения для нового деплоя",
        "почини все проблемы с правами в {service}",
        "обнови все секреты в деплое",
        "исправь все проблемы с сетевой конфигурацией",
        "обнови все лимиты ресурсов {k8s} в кластере",
        "мигрируй все конфиги {k8s} в новый namespace",
        "почини все health check'и {k8s}",
        "обнови все скрипты {infra} под окружение {env}",
        "разреши все проблемы с SSL сертификатами",
    ],

    ("moderate", "mutative", "general"): [
        "перепиши этот раздел",
        "отредактируй весь документ",
        "обнови все примеры в руководстве",
        "перепиши введение и заключение",
        "исправь все ошибки в черновике",
        "обнови план с учётом новых ограничений",
        "перепиши проектное предложение",
        "исправь все несостыковки в документе",
        "обнови все вехи в плане",
        "почини все битые ссылки",
        "перепиши онбординг-гайд",
        "обнови все описания процессов",
    ],

    # ── MODERATE + GENERATIVE ───────────────────────────────────────────────────

    ("moderate", "generative", "coding"): [
        "добавь rate limiter в API {module}",
        "напиши тесты на весь модуль {module}",
        "реализуй пагинацию для всех эндпоинтов {endpoint}",
        "добавь логирование по всему сервису {module}",
        "сделай слой кэширования для {module}",
        "реализуй retry с backoff",
        "добавь валидацию входных данных в хендлеры {module}",
        "напиши интеграционные тесты для {module}",
        "реализуй фичу {feature} в {module}",
        "сделай CLI для {module}",
        "реализуй полный CRUD для {class_name}",
        "добавь сбор метрик в {module}",
        "создай REST API для {module}",
        "добавь аудит-логирование во все эндпоинты {module}",
        "реализуй ролевую модель доступа для {module}",
    ],

    ("moderate", "generative", "support"): [
        "помоги поднять полное окружение {service}",
        "проведи меня через весь процесс настройки {service}",
        "напиши гайд по установке {service}",
        "собери полную конфигурацию для {service}",
        "напиши runbook для деплоя {service}",
        "составь чеклист деплоя для {service}",
        "напиши гайд по миграции с {service} на {service}",
        "напиши troubleshooting-гайд по типовым проблемам {service}",
        "собери набор манифестов {k8s} для {service}",
        "напиши постмортем-шаблон для {incident}",
        "составь гайд по тюнингу производительности {service}",
        "напиши он-колл гайд по {incident}",
    ],

    ("moderate", "generative", "general"): [
        "напиши статью про {topic}",
        "сделай презентацию о {topic}",
        "набросай проектное предложение по {topic}",
        "напиши туториал по {topic}",
        "составь гайд по всем аспектам {topic}",
        "подготовь FAQ по {topic}",
        "напиши развёрнутый пост про {topic}",
        "составь план курса по {topic}",
        "напиши технический обзор {topic}",
        "подготовь полный {doc_type} для этого проекта",
        "составь {work_product} для команды",
        "напиши {doc_type} с объяснением нового процесса для {audience}",
    ],

    # ── MODERATE + READONLY ─────────────────────────────────────────────────────

    ("moderate", "readonly", "coding"): [
        "сделай ревью всего модуля {module}",
        "проверь безопасность всех файлов в {module}",
        "проанализируй производительность {module}",
        "проверь покрытие тестами в {module}",
        "поищи баги по всему {module}",
        "найди все N+1 запросы в {module}",
        "найди все использования {func} в кодовой базе",
        "проверь обработку ошибок в {module}",
        "проанализируй все запросы к базе в {module}",
        "проверь все флоу аутентификации в {module}",
        "оцени качество кода в {module}",
        "проверь зависимости {module}",
    ],

    ("moderate", "readonly", "support"): [
        "объясни весь флоу аутентификации {service}",
        "проведи меня по тому, как {service} работает от и до",
        "помоги разобраться во всей системе {module}",
        "проследи путь запроса через {module}",
        "объясни все режимы отказа {service}",
        "опиши весь поток данных в {module}",
        "объясни весь жизненный цикл запроса {service}",
        "опиши все опции конфигурации {service}",
        "разбери, что произошло во время {incident}",
        "проследи корневую причину {incident}",
        "проведи меня по всем ресурсам {k8s} в кластере",
        "объясни весь пайплайн {infra}",
    ],

    ("moderate", "readonly", "general"): [
        "объясни {topic} в деталях",
        "дай подробный обзор {topic}",
        "опиши все аспекты {topic}",
        "проанализируй плюсы и минусы {topic}",
        "сравни подходы к {topic}",
        "дай развёрнутое объяснение {term}",
        "опиши все компромиссы {topic}",
        "разбери все варианты использования {term}",
        "проведи меня по всем разделам этого {doc_type}",
        "проанализируй все риски в этом {doc_type}",
        "разбери все OKR за этот квартал",
        "объясни весь контекст этого {doc_type}",
    ],

    # ── EXPANSIVE + MUTATIVE ────────────────────────────────────────────────────

    ("expansive", "mutative", "coding"): [
        "перепиши весь {module} с нуля",
        "мигрируй всю кодовую базу на TypeScript",
        "отрефактори всю систему аутентификации",
        "перепиши весь слой работы с базой",
        "переведи всё приложение на async",
        "обнови все зависимости во всех пакетах",
        "замени всё глобальное состояние по всему проекту",
        "приведи логирование к единому виду во всех модулях",
        "мигрируй всё с Python 2 на Python 3",
        "отрефактори весь тестовый набор",
        "замени каждое использование {func} на {func2} по всему проекту",
        "вычисти весь мёртвый код по всему проекту",
        "перепиши весь пайплайн данных с нуля",
        "мигрируй весь бэкенд на микросервисы",
        "переработай всю модель безопасности по всей кодовой базе",
    ],

    ("expansive", "mutative", "support"): [
        "переработай всю мою конфигурацию инфраструктуры",
        "мигрируй весь мой стек на kubernetes",
        "переделай весь деплой с нуля",
        "перестрой весь CI/CD пайплайн",
        "замени весь мой мониторинг",
        "мигрируй всё на нового облачного провайдера",
        "переделай всю сетевую конфигурацию",
        "перестрой всю стратегию деплоя",
        "мигрируй все сервисы в контейнеры",
        "переделай все конфигурации окружений с нуля",
        "замени весь стек логирования",
        "переработай весь он-колл процесс",
    ],

    ("expansive", "mutative", "general"): [
        "переработай всю документацию",
        "перепиши всё с нуля",
        "обнови весь контент на сайте",
        "перестрой всю базу знаний",
        "переделай все материалы онбординга",
        "перепиши весь корпоративный вики",
        "переработай весь обучающий контент",
        "перепиши все регламенты",
        "переделай каждый шаблон в организации",
        "перепиши весь справочник разработчика",
        "перестрой всю документацию по процессам",
        "переработай весь плейбук коммуникаций",
    ],

    # ── EXPANSIVE + GENERATIVE ──────────────────────────────────────────────────

    ("expansive", "generative", "coding"): [
        "реализуй полный CI/CD пайплайн с нуля",
        "собери полную систему аутентификации и авторизации",
        "создай полный тестовый набор, покрывающий всё",
        "сгенерируй полный SDK для всего API",
        "реализуй весь флоу обработки платежей",
        "собери полную плагинную архитектуру",
        "создай полный стек наблюдаемости",
        "реализуй всю событийную систему целиком",
        "собери целую платформу микросервисов",
        "создай полноценное приложение с нуля",
        "заскаффолди всю структуру проекта",
        "сгенерируй весь бойлерплейт для нового сервиса",
        "собери полный пайплайн обработки данных",
        "реализуй всё, что нужно для продакшн-деплоя",
        "создай полный фреймворк загрузки данных",
    ],

    ("expansive", "generative", "support"): [
        "напиши все runbook'и {infra} для обработки {incident}",
        "собери полные доки для всей платформы",
        "напиши все скрипты {infra} для поднятия {env}",
        "создай полный гайд администрирования {service}",
        "напиши полный набор гайдов по эксплуатации всех сервисов",
        "собери полный disaster recovery план для всей инфраструктуры",
        "создай все манифесты {k8s} для всей системы",
        "напиши полную документацию по всем окружениям",
        "собери полный он-колл справочник для всех инцидентов",
        "создай с нуля весь набор конфигураций для {env}",
    ],

    ("expansive", "generative", "general"): [
        "напиши всё, что мне нужно знать о {topic}",
        "создай полный курс по {topic}",
        "подготовь исчерпывающее руководство по {topic}",
        "напиши целую книгу про {topic}",
        "создай каждый {doc_type} и {work_product}, которые нужны для запуска",
        "подготовь полную стратегию по {topic}",
        "собери полную базу знаний по {topic}",
        "напиши весь учебный план по {topic}",
        "создай полный справочник по {term}",
        "подготовь все документы для {audience} с нуля",
    ],

    # ── EXPANSIVE + READONLY ────────────────────────────────────────────────────

    ("expansive", "readonly", "coding"): [
        "сделай ревью всей кодовой базы",
        "проведи аудит безопасности всего проекта",
        "проанализируй архитектуру всей системы",
        "найди все уязвимости по всему проекту",
        "проверь всё покрытие тестами по всем модулям",
        "дай полную картину всей системы",
        "проанализируй производительность каждого сервиса",
        "проследи все потоки данных через всю систему",
        "проверь каждую зависимость во всех пакетах",
        "разбери всю кодовую базу от и до",
    ],

    ("expansive", "readonly", "support"): [
        "объясни всю инфраструктуру целиком",
        "опиши каждый компонент во всех окружениях",
        "разбери весь стек от фронта до базы",
        "проведи полный аудит всей конфигурации",
        "проследи все зависимости между всеми сервисами",
        "опиши всю сетевую топологию целиком",
        "разбери каждый {k8s} во всех кластерах",
        "объясни весь жизненный цикл всех деплоев",
        "проанализируй все {log} по всем сервисам",
        "дай полную картину всего продакшена",
    ],

    ("expansive", "readonly", "general"): [
        "объясни всё о {topic}",
        "дай самое полное объяснение {topic}",
        "разбери {topic} исчерпывающе от и до",
        "опиши весь ландшафт {topic}",
        "сравни все подходы и точки зрения на {topic}",
        "расскажи всё, что вообще известно о {topic}",
        "сделай обзор всех перспектив по {topic}",
        "разбери каждый аспект {topic}",
        "опиши полную историю {topic}",
        "проанализируй все возможные сценарии для {topic}",
    ],
}
...
# ── Russian hard eval — phrasing not present in _TEMPLATES_RU ───────────────────

_HARD_EVAL_RU: list[tuple[str, str, str, str]] = [
    # ── coding ──────────────────────────────────────────────────────────────────
    ("focused",   "mutative",   "coding",    "в {func} баг, почини пожалуйста"),
    ("focused",   "mutative",   "coding",    "{func} не обрабатывает null, исправь это"),
    ("focused",   "mutative",   "coding",    "тут опечатка в {file}, поправь её"),
    ("focused",   "readonly",   "coding",    "не понимаю, что делает {func}, растолкуй"),
    ("focused",   "readonly",   "coding",    "глянь на {func} — зачем она вообще нужна"),
    ("focused",   "generative", "coding",    "накидай тестик на {func}"),
    ("focused",   "generative", "coding",    "нужен мок для {class_name}, сделаешь"),
    ("moderate",  "mutative",   "coding",    "пройдись по {module} и подчисти код"),
    ("moderate",  "mutative",   "coding",    "{module} не дружит с новым API, обнови его"),
    ("moderate",  "readonly",   "coding",    "посмотри модуль {module} целиком на предмет багов"),
    ("moderate",  "generative", "coding",    "нужны тесты на весь {module}"),
    ("expansive", "mutative",   "coding",    "пройдись по всему проекту и приведи всё в порядок"),
    ("expansive", "readonly",   "coding",    "дай общую картину всей системы целиком"),
    ("expansive", "generative", "coding",    "подними полный CI/CD пайплайн с нуля"),
    # ── support ─────────────────────────────────────────────────────────────────
    ("focused",   "readonly",   "support",   "что за ошибка {error}, откуда она берётся"),
    ("focused",   "mutative",   "support",   "конфиг {service} кривой, поправь"),
    ("focused",   "generative", "support",   "скинь команду перезапуска {service}"),
    ("moderate",  "readonly",   "support",   "разберись, из-за чего случился {incident}"),
    ("moderate",  "generative", "support",   "собери подробную инструкцию по настройке {service}"),
    ("expansive", "generative", "support",   "нужны доки на всю платформу целиком"),
    # ── general ─────────────────────────────────────────────────────────────────
    ("focused",   "readonly",   "general",   "а {term} — это вообще что"),
    ("focused",   "generative", "general",   "черкни пару строк про {topic}"),
    ("moderate",  "generative", "general",   "подготовь развёрнутый материал про {topic}"),
    ("moderate",  "mutative",   "general",   "перелопать этот раздел заново"),
    ("expansive", "readonly",   "general",   "расскажи про {topic} вообще всё, что есть"),
    ("expansive", "mutative",   "general",   "переделай документацию полностью, всю"),
]
...
def generate(
    seed: int = 42,
    target_per_combo: int = 500,
    ru_per_combo: int = 250,
) -> list[tuple[str, str, str, str]]:
    """
    Training corpus. Returns (text, complexity, nature, domain) tuples.
    target_per_combo examples generated per English label combination,
    ru_per_combo per Russian combination (_TEMPLATES_RU covers a subset of
    domains but all complexity × nature cells).
    ~45 combos × 500 + ~24 RU combos × 250 ≈ 28,500 base + augmentation.
    """
    ...
    for (complexity, nature, domain), templates in _TEMPLATES_RU.items():
        count = 0
        while count < ru_per_combo:
            tpl = rng.choice(templates)
            text = _sub(tpl, rng)
            text = _augment_ru(text, rng)
            out.append((text, complexity, nature, domain))
            count += 1
...
def generate_hard(seed: int = 42) -> list[tuple[str, str, str, str]]:
    """
    Hard eval set (English + Russian). Phrases not derived from any training
    template. Use ONLY for evaluation — never include in training data.
    """
    rng = random.Random(seed)
    out: list[tuple[str, str, str, str]] = []
    for complexity, nature, domain, template in _HARD_EVAL + _HARD_EVAL_RU:
